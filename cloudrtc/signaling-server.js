...
function signaling_server (socket, config) {
    ...
    function onConnection(socket) {
        ...
        let maxParticipantsAllowed = parseInt(params.maxParticipantsAllowed || 1000) || 1000;
        let enableScalableBroadcast = params.enableScalableBroadcast === true || params.enableScalableBroadcast === 'true';
        ...
        function joinARoom(message) {
            try {
                ...
                if (listOfRooms[roomid].session && (listOfRooms[roomid].session.oneway === true || listOfRooms[roomid].session.broadcast === true)) {
                    let owner = listOfRooms[roomid].owner;
        ...
        socket.on(socketMessageEvent, function(message, callback) {
            ...
            try {
                if (message.remoteUserId && message.remoteUserId != 'system' && message.message.newParticipationRequest) {
                    if (enableScalableBroadcast === true) {
                        let user = listOfUsers[message.remoteUserId];
                        if (user) {
                            user.socket.emit(socketMessageEvent, message);
                        }
